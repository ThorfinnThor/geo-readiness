...
def test_questions_are_generated_in_the_language_the_site_is_written_in() -> None:
    # A generated question stands in for what this site's customers would type
    # into an answer engine, and they type in the site's language. Testing a
    # German business with English questions tests a query nobody makes.
    german = BusinessProfile(
    ...
    clusters = generate_clusters(german, "geo-readiness-v2", "quick", prompt_version="v2")
    assert {c.language for c in clusters} == {"de"}
    texts = [p.prompt_text for c in clusters for p in c.prompts]
    # Lowercased because this profile carries no offering_display map; casing is
    # restored from the crawl, and is not what this test is about.
    assert "Welche Anbieter gibt es für kindertagespflege in Ulm?" in texts

    english = BusinessProfile(
    ...
    clusters = generate_clusters(english, "geo-readiness-v2", "quick", prompt_version="v2")
    assert {c.language for c in clusters} == {"en"}
    texts = [p.prompt_text for c in clusters for p in c.prompts]
    assert "Which providers are there for solar installation in Austin?" in texts


def test_the_dominant_language_wins_on_a_multilingual_site() -> None:
    # `languages` is sorted alphabetically, so its first entry is not the site's
    # main language. The crawl's dominant language is what decides.
    profile = BusinessProfile(
        canonical_domain="besttravelclimate.com",
        brand_name="Best Travel Climate",
        languages=["de", "en", "es", "fr"],
        primary_language="de",
        topics=["Madeira im Oktober"],
    )
    clusters = generate_clusters(profile, "geo-readiness-v2", "quick", prompt_version="v2")
    assert {c.language for c in clusters} == {"de"}
